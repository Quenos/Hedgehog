...
const distributeAmount = (total, weights, isDeribit) => {
  let leftover = 0;
  ...
  weights.forEach(weight => {
    ...
    var weightedValue = 0;
    if (!isDeribit) {
      weightedValue = Math.trunc(val);
      leftover = val % 1;
    } else {
      // Deribit has order size multiple of 10
      weightedValue = Math.trunc(Math.round(val / 10) * 10);
      leftover = val - weightedValue;
    }
